...
def create(request):
    try:
        data = auth(request)
        loyBalance = requests.get(f"http://{ROYALTY_IP}:8000/api/v1/loyalty/balance", cookies=request.COOKIES)
...
def pay(request, payment_uid):
    try:
        ...
        payment.status = "PAID"
        pay_loyalty = requests.patch(f"http://{ROYALTY_IP}:8000/api/v1/loyalty/edit_balance",
