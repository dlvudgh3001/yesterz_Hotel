...
def create_or_all(request):
    ...
    try:
        ...
        if request.method == 'POST':
            payBalance = requests.post(f"http://{PAYMENT_IP}:8002/api/v1/payment/create",
            ...
            for res in users_reservations:
                payBalance = requests.get(
                    f"http://{PAYMENT_IP}:8002/api/v1/payment/status/{{}}".format(res['fields'].get("payment_uid")),
                    cookies=request.COOKIES)
                if payBalance.status_code == 200:
                    payBalance = payBalance.json()
                    res['fields'].update(payBalance)
                hotel = requests.get(
                    f"http://{HOTEL_IP}:8004/api/v1/hotels/status/{{}}".format(res['fields'].get("hotel_uid")),
                    cookies=request.COOKIES)
...
def canceled(request, booking_uid):
    try:
        ...
        payStatus = requests.delete(f"http://{PAYMENT_IP}:8002/api/v1/payment/close/{{}}".format(payment_uid),
                                    cookies=request.COOKIES)
...
def pay(request, booking_uid):
    try:
        auth(request)
        payment_uid = Reservations.objects.get(booking_uid=booking_uid).payment_uid
        payStatus = requests.post(f"http://{PAYMENT_IP}:8002/api/v1/payment/pay/{{}}".format(payment_uid),
                                  cookies=request.COOKIES)
        if payStatus.status_code == 200:
            return JsonResponse(payStatus.json(), status=status.HTTP_200_OK)
...
def reversed(request, booking_uid):
    try:
        auth(request)
        payment_uid = Reservations.objects.get(booking_uid=booking_uid).payment_uid
        payStatus = requests.post(f"http://{PAYMENT_IP}:8002/api/v1/payment/reversed/{{}}".format(payment_uid),
                                  cookies=request.COOKIES)
        if payStatus.status_code == 200:
            return JsonResponse(payStatus.json(), status=status.HTTP_200_OK)
...
def about_one(request, booking_uid):
    try:
        ...
        hotel = requests.get(f"http://{HOTEL_IP}:8004/api/v1/hotels/status/{{}}".format(reservations["hotel_uid"]),
                             cookies=request.COOKIES)  # нужно доделать
        if hotel.status_code == 200:
            hotel = hotel.json()
            reservations.update(hotel)
        payBalance = requests.get(f"http://{PAYMENT_IP}:8002/api/v1/payment/status/{{}}".format(reservations["payment_uid"]),
                                  cookies=request.COOKIES)
        if payBalance.status_code == 200:
            payBalance = payBalance.json()
...
def filter_booking(request, date_start, date_end):
    try:
        ...
        for res in reservations:
            payBalance = requests.get(
                f"http://{PAYMENT_IP}:8002/api/v1/payment/status/{{}}".format(res["payment_uid"]),
                cookies=request.COOKIES)
            if payBalance.status_code == 200:
                payBalance = payBalance.json()
    ...
    try:
        ...
        for res in reservations:
            ...
            about_hotel = requests.get(
                f"http://{HOTEL_IP}:8004/api/v1/hotels/{{}}".format(res['fields'].get("hotel_uid")),
                cookies=request.COOKIES)
            if about_hotel.status_code == 200:
                about_hotel = about_hotel.json()
                res['fields'].update(about_hotel)
            user = requests.get(
                f"http://{SESSION_IP}:8001/api/v1/session/user/{{}}".format(res['fields'].get("user_uid")),
                cookies=request.COOKIES)
            if user.status_code == 200:
                user = user.json()
                res['fields'].update(user)
            loyalty = requests.get(
                f"http://{ROYALTY_IP}:8000/api/v1/loyalty/status/{{}}".format(res['fields'].get("user_uid")),
                cookies=request.COOKIES)
